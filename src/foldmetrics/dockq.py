...
def compute_dockq(
    ...
    (label vs auth asym ids), so check the reported pairing.
    ...
    if mapping is not None:
        ...
        chain_map = {native: model for model, native in mapping.items()}
    elif set(model_chains) == set(native_chains):
        chain_map = {c: c for c in native_chains}
    else:
        chain_map = {
            native: model
    ...
    result, total = run_all(
        model, native, chain_map=chain_map, capri_peptide=capri_peptide
    )
